    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                expect(page.get_by_role("tab", name="Đơn Bán Hàng")).to_have_attribute("aria-selected", "true")
                runner.log("WF-09", 1, "PASS", "Truy cập /sales thành công, hiển thị tab mặc định Đơn Bán Hàng", url=page.url)
            ...
            try:
                page.goto(f"{BASE_URL}/sales?tab=invoices")
                wait_for_page_ready(page)
                expect(page.get_by_role("tab", name="Hóa Đơn Bán")).to_have_attribute("aria-selected", "true")
                
                page.reload()
                wait_for_page_ready(page)
                expect(page.get_by_role("tab", name="Hóa Đơn Bán")).to_have_attribute("aria-selected", "true")
                
                # Quay trở lại tab Đơn Bán Hàng
                page.get_by_role("tab", name="Đơn Bán Hàng").click()
                time.sleep(0.5)
                runner.log("WF-09", 2, "PASS", "Tab đồng bộ chính xác qua URL (?tab=invoices) và duy trì trạng thái sau khi reload", url=page.url)
            except Exception as e:
            ...
            try:
                ...
                page.locator("select").first.select_option(label="Siêu thị Điện máy & Đồ gia dụng GreenMart (KH003)")
                page.locator("select").nth(1).select_option(label="Bóng đèn Huỳnh Quang 1m2 36W (TP_HQ01)")
            ...
            try:
                ...
                runner.log("WF-09", 7, "PASS", "Xác thực hóa đơn bán được tự động tạo với trạng thái 'Chưa Thanh Toán' thành công", url=page.url)
            except Exception as e:
