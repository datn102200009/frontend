  ...
  const onSubmit = async (data: PurchaseOrderInput) => {
    try {
      if (orderId) {
        ...
        await createOrder({ purchaseOrderInput: { ...data, status: 'draft' } }).unwrap();
      }
      onSuccess();
    } catch (err) {
        ...
        footer={
          <div style={{ display: 'flex', width: '100%', justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              {orderId && isDraft && (
                <Button variant="outline" onClick={handleConfirm} loading={isApproving} disabled={isWorking} icon={<CheckCircle size={16} />}>
                  Duyệt Đơn
                </Button>
              )}
              {orderId && isPending && (
          ...
          <form className={styles.form} onSubmit={(e) => e.preventDefault()}>
            ...
            <div className={styles.row}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-1)', flex: 1 }}>
                ...
                <select id="vendor_id" className={styles.itemInput} {...register('vendor_id', { required: 'Bắt buộc' })} disabled={isWorking || isReadOnly}>
              ...
              <div className={styles.itemsTable}>
                ...
                {fields.map((field, index) => (
                  <div key={field.id} className={styles.itemRow} style={{ padding: '8px 0', gridTemplateColumns: isReadOnly ? '1fr 100px 150px' : '1fr 100px 150px 36px' }}>
                    ...
                    <input className={styles.itemInput} type="number" min={0.01} step={0.01} {...register(`lines.${index}.quantity` as const, { valueAsNumber: true, required: true, min: 0.01 })} disabled={isWorking || isReadOnly} />
