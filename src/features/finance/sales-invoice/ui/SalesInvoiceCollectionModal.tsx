...
export const SalesInvoiceCollectionModal: React.FC<SalesInvoiceCollectionModalProps> = ({
  ...
  return (
    <>
      ...
      <ConfirmModal
        open={showConfirm}
        title="Xác nhận thu tiền hóa đơn"
        message={
          <div>
            Bạn có chắc chắn muốn thu số tiền <strong>{formatCurrency(collectAmount)}</strong>{customerName ? ` từ khách hàng ${customerName}` : ''} cho hóa đơn bán hàng này không? Dòng tiền thu sẽ được tạo và ghi nhận hoàn tất ngay lập tức.
          </div>
        }
